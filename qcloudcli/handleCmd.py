...
class QcloudCredentials(object):
    ...
    for path in sys.path:
        if path and os.path.isdir(path):
            ...
            for object in objects:
                if not object.startswith('qcloudsdk'):
                    continue
                if os.path.isdir(os.path.join(path, object)):
                    return path


class handleCmd(object):
    def __init__(self):
    ...
    def showQcloudCliHelp(self):
        ...
        if not os.path.isdir(ConfigurePath):
            print("%s\nThis is the first time you use qcloudcli, "
                  "please use command 'qcloudcli configure' to set "
                  "your personal information.%s" % (self.red, self.end))
        print(self.begin + "\nNAME:" + self.end)
        print("\tqcloudcli")
        print(self.begin + "\nDESCRIPTION:" + self.end)
        print("\n\tThe Qcloud Command Line Interface is a unified tool "
              "to manage your qcloud services.")
        print(self.begin + "\nCONFIGURE:" + self.end)
        print("\n\tBefore using qcloudcli, you should use the command "
              "as follow:")
        print(self.red + "\n\tqcloudcli configure" + self.end)
        print("\n\tThis command will save your SecretId and SecretKey "
              "and the default output format (json text or table).")
        print(self.begin + "\nFORMATE:" + self.end)
        print("\n\tYou should use qcloudcli as this format:\n")
        print("%s\tqcloudcli <module> <action> [options and parameters]%s"
              % (self.red, self.end))
        print("\n\tYou can check our site to see the detail of using "
              "qcloudcli.")
        print(self.begin + "\nAVAILABLE MODULES:" + self.end + "\n")
        modules = self.getAllmodules()
        self.showAsTwoLines(modules)
        print(self.begin + "\nAVAILABLE ACTIONS:" + self.end + "\n")
        print("\tYou can use the command %sqcloudcli <module> --help%s to "
    ...
    def getAllmodules(self):
        ...
        if all_python_packages is not None:
            for package in all_python_packages:
                path = os.path.join(site_packages_path, package)
                if not os.path.isdir(path):
                    continue
                if package.startswith('qcloudsdk'):
    ...
    def showAsTwoLines(self, data):
        ...
        for item in mlist2:
            ...
            count = count+1
            if len(tmpList) == 2:
                print('\t%s\t|%s' % ('{0:30}'.format(tmpList[0]),
                                     format(tmpList[1], '<10')))
                tmpList = list()
